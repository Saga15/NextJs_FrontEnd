...
export const DisplayViewComponent = ({ display, children }) => {
    ...
    <>
      <div className={`table-projects table-responsive ${tableContainerClass}`}>
        {bulkActions && bulkActions.length > 0 && rows.length > 0 && (
            <div
              className={`mb-2 mt-2 bulk-action-apply d-flex ${dropdownContainerClass}`}
            >
              <select
                style={{ width: "150px" }}
              ...
              >
                ...
                {bulkActions.map((action, index) => (
                  <option key={index+1} value={index}>{action?.actionTitle}</option>
                ))}
              </select>
